...
public class DomainClassifier {
	...
	private Double[] calculateDomainScores(String nowInputText) {
	    ...
	    for (String weatherKeyword : weatherDictionary) {
	        if (nowInputText.toLowerCase().contains(weatherKeyword)) {
	            // {"Other", "Weather", "Food"}, so scoreArray[1] indicates the score for Weather domain
	            scoreArray[1] = scoreArray[1] + 1.0;
	        }
	    }
...
String[] weatherDictionary = new String[] {"snow", "rain", "weather",
"rain", "snow"};
Set<String> keywordSet = new
HashSet<>(Arrays.asList(weatherDictionary));
...
int nowIndex =
nowInputText.toLowerCase().indexOf(foodKeyword);
scoreArray[1] = scoreArray[1].doubleValue()+1.0+
(double)nowIndex;
}
}
...
"(should|do)[\\s]+i[\\s]+(carry|bring|take|need)[\\s]+an[\\
s]+umbrella[\\s|\\.|?]*"
};
for(String nowPatternStr: patternArray) {
Pattern nowPattern = Pattern.compile(nowPatternStr);
//the patterns use lower case, so the input needs to convert to
lower cases
Matcher nowMatcher =
nowPattern.matcher(nowInputText.trim().toLowerCase());
...
scoreArray[1] = 100.00;
	...
	public String getLabel(String nowInputText) {
		...
		for(int i=0;i<intentScores.length;i++){
			...
			if(nowMaxScore==null||nowMaxIndex==-1||
					intentScores[i].doubleValue()>nowMaxScore.doubleValue()) {
				nowMaxIndex = i;
