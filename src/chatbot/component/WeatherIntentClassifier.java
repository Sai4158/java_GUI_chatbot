...
public class WeatherIntentClassifier {
	...
	private Double[] calculateIntentScores(String nowInputText) {
		...
		// Assign scores based on keyword relevance for weather intents
		if (nowInputText.toLowerCase().contains("report") || nowInputText.toLowerCase().contains("weather")) {
		    scoreArray[0] = 1.0;  // Score for "WeatherReport" intent
		}
		if (nowInputText.toLowerCase().contains("snow")) {
		    scoreArray[1] = 1.0;  // Score for "Snow" intent
		}
		if (nowInputText.toLowerCase().contains("rain")) {
		    scoreArray[2] = 1.0;  // Score for "Rain" intent
		}
